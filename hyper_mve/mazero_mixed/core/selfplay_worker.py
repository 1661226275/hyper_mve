...
def oracle_blend_weight(config, trained_steps: int) -> float:
    """Three-stage belief curriculum weight (oracle -> linear anneal -> pure
    inference): w=1 for the first belief_oracle_steps, linearly annealed to 0
    over belief_anneal_steps, then 0."""
    s1 = getattr(config, "belief_oracle_steps", 0)
    s2 = getattr(config, "belief_anneal_steps", 0)
    if trained_steps < s1:
        return 1.0
    if trained_steps < s1 + s2:
        return 1.0 - (trained_steps - s1) / max(float(s2), 1.0)
    return 0.0
...
class DataWorker(object):
    def __init__(self, rank, config: BaseConfig, replay_buffer: ReplayBuffer, shared_storage: SharedStorage):
        ...
        # per-env belief-GRU hidden states (subjective model only)
        self.belief_hiddens = None
        if getattr(self.config, "subjective_model", False):
            self.belief_hiddens = np.zeros(
                (self.config.num_pmcts, self.config.num_agents,
                 self.model.belief_net.gru_hidden),
                dtype=np.float32,
            )
    ...
    def init_envs(self):
        ...
        # subjective model: data-collection envs run with the oracle regime id
        # in info (train-time-only privileged info for belief supervision).
        self.envs = [
            self.config.new_game(
                self.config.seed + (self.rank + 1) * i,
                oracle=getattr(self.config, "subjective_model", False),
            )
    ...
    def reset_env(self, env_id):
        ...
        if self.belief_hiddens is not None:
            self.belief_hiddens[env_id] = 0.0
    ...
    def run(self, start_training: bool = False, trained_steps: int = 0) -> int:
        ...
        with torch.no_grad():
            ...
            while episodes_collected < num_envs:
                ...
                # subjective model: one belief-GRU step per real env step; the
                # curriculum blends oracle one-hot with the inferred posterior.
                blend_np, prev_hidden_np, g_true_arr = None, None, None
                if self.belief_hiddens is not None:
                    E, N = num_envs, self.config.num_agents
                    G = self.model.n_regimes
                    obs_flat = stack_obs.reshape(E, N, -1)
                    prev_hidden = torch.from_numpy(self.belief_hiddens).to(self.device)
                    new_hidden, g_hat = self.model.belief_net.step(obs_flat, prev_hidden)
                    g_true_arr = np.array([env.oracle_g() for env in self.envs])
                    w = oracle_blend_weight(self.config, trained_steps)
                    if w > 0.0 and (g_true_arr >= 0).all():
                        onehot = torch.nn.functional.one_hot(
                            torch.from_numpy(g_true_arr).long(), G
                        ).float().to(self.device)                       # (E, G)
                        onehot = onehot.unsqueeze(1).expand(E, N, G)
                        blend = w * onehot + (1.0 - w) * g_hat
                    else:
                        blend = g_hat
                    self.model.set_belief(blend, step=trained_steps)
                    prev_hidden_np = self.belief_hiddens.copy()
                    self.belief_hiddens = new_hidden.detach().cpu().numpy()
                    blend_np = blend.detach().cpu().numpy()
                ...
                for i in range(num_envs):
                    ...
                    pred_value = float(np.mean(network_output.value[i]))
                    ...
                    self.eps_reward_lst[i] += float(np.mean(reward))
