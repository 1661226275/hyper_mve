...
class DynamicsNetwork(nn.Module):
    def __init__(
        ...
        self.fc_reward = mlp(num_agents * (hidden_state_size + action_space_size),
                             fc_reward_layers, num_agents * full_support_size, use_value_out=True)
    ...
    def forward(self, hidden_state: torch.Tensor, action: torch.Tensor):
        ...
        reward = self.fc_reward(
            torch.cat([state, action], dim=2).reshape(batch_size, -1)
        ).reshape(batch_size, self.num_agents, self.full_support_size)
...
class PredictionNetwork(nn.Module):
    def __init__(
        ...
        self.full_support_size = full_support_size
        # per-agent value heads over the centralized (global) input (A3 migration).
        self.fc_value = mlp(hidden_state_size * num_agents, fc_value_layers, num_agents * full_support_size, use_value_out=True)
        self.fc_policy = mlp(hidden_state_size, fc_policy_layers, action_space_size, use_value_out=True)

    def forward(self, x):
        value = self.fc_value(x).reshape(-1, self.num_agents, self.full_support_size)
...
class MAMuZeroNet(BaseNet):
    ...
    def initial_inference(self, observation: torch.Tensor) -> NetworkOutput:
        ...
        return NetworkOutput(hidden_state, np.zeros((batch_size, self.num_agents, 1)), value, policy_logit)
