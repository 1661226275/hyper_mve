...
def parse_args(args):
    ...
    groups.add_argument("--case", required=True, type=str, choices=["smac", "matrix", "lunarlander", "bandit", "mujoco", "gfootball", "relation"],
    ...
    groups.add_argument("--decoupled_selection", action="store_true", default=False,
                        help="By default False (joint team-UCB over sampled children); "
                             "if set, each agent independently maximizes its own UCB over "
                             "marginal statistics (decoupled UCT, mixed-game mode).")
    groups.add_argument("--subjective_model", action="store_true", default=False,
    ...
    groups.add_argument("--belief_anneal_steps", type=int, default=3000,
                        help="Curriculum stage 2: linear anneal from oracle to pure inference.")
    groups.add_argument("--belief_loss_coeff", type=float, default=1.0,
...
class BaseConfig(ABC):
    ...
    def _support_loss(self, prediction: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        # Shape-agnostic scalar/vector loss reducing all trailing dims to (batch,).
        # scalar mode:  pred (B, N, 1)   target (B, N)     -> MSE, sum over agents
        # vector mode:  pred (B, N, S)   target (B, N, S)  -> CE over support, sum over agents
        if not self.use_vectorization:
            loss = torch.nn.MSELoss(reduction='none')(prediction, target.unsqueeze(-1)).squeeze(-1)
        else:
            loss = -(torch.log_softmax(prediction, dim=-1) * target).sum(-1)
        while loss.dim() > 1:
            loss = loss.sum(-1)
        return loss

    def reward_loss(self, prediction: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        return self._support_loss(prediction, target)
