...
class GameHistory:
    ...
    def store_context(self, belief, g_true, belief_hidden):
        ...
        Parameters
        ----------
        belief : np.ndarray (N, |G|)
            the (possibly curriculum-blended) posterior used to build ctx
        g_true : int
            oracle regime id (-1 when unavailable)
        belief_hidden : np.ndarray (N, gru_hidden)
            belief-GRU hidden BEFORE this step's update
        """
        self.beliefs.append(belief)
        self.g_trues.append(g_true)
        self.belief_hiddens.append(belief_hidden)
    ...
    def game_over(self):
        ...
        if len(self.beliefs) > 0:
            self.beliefs = np.array(self.beliefs)              # (T, N, |G|)
            self.g_trues = np.array(self.g_trues)              # (T,)
            self.belief_hiddens = np.array(self.belief_hiddens)  # (T, N, 128)
