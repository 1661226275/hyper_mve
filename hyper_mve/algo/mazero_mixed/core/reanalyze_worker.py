...
class ReanalyzeWorker(object):
    ...
    def _prepare_reward_value_re(
        ...
        if self.config.use_root_value:
            ...
            value_lst = np.asarray(search_results.value_vec).reshape(-1, self.config.num_agents)
        # use the predicted values
        elif self.config.use_pred_value:
            value_lst = np.asarray(network_output.value).reshape(-1, self.config.num_agents)
        else:
            raise NotImplementedError

        # get last state discounted value — per-agent (M, N)
        value_lst = value_lst * np.power(self.config.discount, td_steps_lst)[:, None]
        value_lst = value_lst * np.asarray(value_mask)[:, None]
        ...
        for traj_len, reward_lst, state_index in zip(traj_lens, rewards_lst, game_pos_lst):
            ...
            for current_index in range(state_index, state_index + self.config.num_unroll_steps + 1):
                ...
                if current_index < traj_len:
                    ...
                    target_values.append(np.zeros(self.config.num_agents, dtype=np.float32))
                    target_rewards.append(np.zeros(self.config.num_agents, dtype=np.float32))
                value_index += 1

            batch_rewards.append(target_rewards)
            batch_values.append(target_values)

        batch_rewards = np.asarray(batch_rewards).reshape(self.config.batch_size, self.config.num_unroll_steps + 1, self.config.num_agents)
        batch_values = np.asarray(batch_values).reshape(self.config.batch_size, self.config.num_unroll_steps + 1, self.config.num_agents)
            ...
            for current_index in range(state_index, state_index + self.config.num_unroll_steps + 1):
                if current_index < traj_len:
                    ...
                    if bootstrap_index < traj_len:
                        ...
                        bootstrap_value = 0
                    # stored root/pred values are agent-mean scalars; broadcast
                    # to a per-agent vector before accumulating vector rewards.
                    bootstrap_value = np.full(self.config.num_agents, bootstrap_value, dtype=np.float32)
                    for i, reward in enumerate(reward_lst[current_index:bootstrap_index]):
    ...
    def _prepare_policy_re(
        ...
        for game, state_index in zip(games, game_pos_lst):
            ...
            for current_index in range(state_index, state_index + K + 1):
                if current_index < traj_len:
                    ...
                    obs = game_obs[beg_index:end_index]
                    if subjective:
                        belief_lst.append(game.beliefs[current_index])
                else:
        ...
        # per-agent q-values (rows, C, num_agents) and root values (rows, 1, num_agents)
        batch_sampled_qvalues_re, _ = concat_with_zero_padding(search_results.sampled_qvalues_vec, C)
        batch_root_mcts_values_re = np.expand_dims(np.asarray(search_results.value_vec), axis=1)
        batch_root_pred_values_re = np.expand_dims(
            np.asarray(network_output.value).reshape(-1, self.config.num_agents), axis=1
        )
    ...
    def make_batch(
        ...
        for game, state_index in zip(game_lst, game_pos_lst):
            ...
            if subjective:
                ...
                g_true_lst.append(game.g_trues[state_index])
        ...
        batch_sampled_qvalues = np.empty((0, C, N))
        batch_root_mcts_values = np.empty((0, 1, N))
        batch_root_pred_values = np.empty((0, 1, N))
        ...
        # (6) compute and normalize the per-agent advantage (rows, C, N):
        # each agent's advantage is its own q minus its own root predicted
        # value, normalized by its own advantage statistics (A5 migration).
        ...
        adv_mean = np.nanmean(adv_copy, axis=(0, 1))
        adv_std = np.nanstd(adv_copy, axis=(0, 1))
