...
cdef extern from "lib/cnode.h" namespace "tree":
    ...
    cdef cppclass CTree_batch:
        ...
        void get_roots_values(float* p)                 # shape = (batch_size, ) — agent-mean
        void get_roots_values_vec(float* p)             # shape = (batch_size, num_agents)
        ...
        void get_root_sampled_pred_values_vec(int tree_id, float* p)                # shape = (deg(root), num_agents)
        void get_root_sampled_mcts_values_vec(int tree_id, float* p)                # shape = (deg(root), num_agents)
        void get_root_sampled_rewards_vec(int tree_id, float* p)                    # shape = (deg(root), num_agents)
        void get_root_sampled_qvalues_vec(int tree_id, float* p, float discount)    # shape = (deg(root), num_agents)
