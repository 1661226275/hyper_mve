...
cdef class Tree_batch:
    ...
    def get_roots_values_vec(self):
        root_values = np.empty(self.root_num * self.agent_num, order='C', dtype='float32')
        cdef float[::1] arr = root_values
        self.trees[0].get_roots_values_vec(&arr[0])
        return root_values.reshape(self.root_num, self.agent_num)
    ...
    def get_roots_sampled_pred_values_vec(self):
        res = []
        cdef int num_children_of_root
        cdef float[::1] arr
        for i in range(self.root_num):
            num_children_of_root = self.trees[0].get_num_children_of_root(i)
            buf = np.empty(num_children_of_root * self.agent_num, order='C', dtype='float32')
            arr = buf
            self.trees[0].get_root_sampled_pred_values_vec(i, &arr[0])
            res.append(buf.reshape(num_children_of_root, self.agent_num))
        return res

    def get_roots_sampled_mcts_values_vec(self):
        res = []
        cdef int num_children_of_root
        cdef float[::1] arr
        for i in range(self.root_num):
            ...
            buf = np.empty(num_children_of_root * self.agent_num, order='C', dtype='float32')
            arr = buf
            self.trees[0].get_root_sampled_mcts_values_vec(i, &arr[0])
            res.append(buf.reshape(num_children_of_root, self.agent_num))
        return res

    def get_roots_sampled_rewards_vec(self):
        res = []
        cdef int num_children_of_root
        cdef float[::1] arr
        for i in range(self.root_num):
            ...
            buf = np.empty(num_children_of_root * self.agent_num, order='C', dtype='float32')
            arr = buf
            self.trees[0].get_root_sampled_rewards_vec(i, &arr[0])
            res.append(buf.reshape(num_children_of_root, self.agent_num))
        return res

    def get_roots_sampled_qvalues_vec(self, float discount):
        res = []
        cdef int num_children_of_root
        cdef float[::1] arr
        for i in range(self.root_num):
            ...
            buf = np.empty(num_children_of_root * self.agent_num, order='C', dtype='float32')
            arr = buf
            self.trees[0].get_root_sampled_qvalues_vec(i, &arr[0], discount)
            res.append(buf.reshape(num_children_of_root, self.agent_num))
        return res
