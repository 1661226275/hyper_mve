"""Vendored from agent/optim/loss.py.

[PORT] relative imports; ``wandb.log`` replaced by the module-level ``LOGGER``
hook (a ``callable(dict)`` or ``None``); no numeric/logic changes. The
imagination-phase per-agent reward mean (``critic_rollout``) is upstream
behaviour and is deliberately preserved (cooperative assumption — disclosed).
"""
import numpy as np
import torch
import torch.nn.functional as F

from .optim_utils import rec_loss, compute_return, state_divergence_loss, calculate_ppo_loss, \
    batch_multi_agent, log_prob_loss, info_loss, FreezeParameters  # [PORT] merged module
from .rnns import rollout_representation, rollout_policy           # [PORT] was: networks.dreamer.rnns

#: [PORT] optional metrics sink replacing wandb.log; set by the learner/runner.
LOGGER = None


def _log(metrics):
    """[PORT] wandb.log replacement — forwards to LOGGER when set."""
...
def model_loss(config, model, obs, action, av_action, reward, done, fake, last):
    ...
    if np.random.randint(20) == 4:
        _log({'Model/reward_loss': reward_loss, 'Model/div': div, 'Model/av_action_loss': av_action_loss,
...
def critic_rollout(model, critic, states, rew_states, actions, raw_states, config):
    with FreezeParameters([model, critic]):
        ...
        _log({'Value/Max reward': imag_reward.max(), 'Value/Min reward': imag_reward.min(),
...
def actor_loss(imag_states, actions, av_actions, old_policy, advantage, actor, ent_weight):
    ...
    if np.random.randint(10) == 9:
        _log({'Policy/Entropy': ent_loss.mean(), 'Policy/Mean action': actions.float().mean()})
