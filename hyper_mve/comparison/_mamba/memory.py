"""Vendored from agent/memory/DreamerMemory.py.

[PORT] ``environments.Env`` removed — av_actions is always allocated/returned
(upstream STARCRAFT branch).
"""
import numpy as np
import torch
...
class DreamerMemory:
    def __init__(self, capacity, sequence_length, action_size, obs_size, n_agents, device):
        self.capacity = capacity
        self.sequence_length = sequence_length
        self.action_size = action_size
        self.obs_size = obs_size
        self.device = device
        self.init_buffer(n_agents)

    def init_buffer(self, n_agents):
        ...
        # [PORT] upstream gates this on env_type == Env.STARCRAFT; always on.
        self.av_actions = np.empty((self.capacity, n_agents, self.action_size), dtype=np.float32)
    ...
    def append(self, obs, action, reward, done, fake, last, av_action):
        if self.actions.shape[-2] != action.shape[-2]:
            self.init_buffer(action.shape[-2])
