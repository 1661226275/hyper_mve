"""Vendored from agent/models/DreamerModel.py.

[PORT] relative imports; ``environments.Env`` removed — the av_action head is
always created (upstream STARCRAFT branch; availability is trained on all-ones
for RelationCommons where every action is always legal).
"""
import torch
import torch.nn as nn

from .dense import DenseBinaryModel, DenseModel   # [PORT] was: networks.dreamer.dense
from .vae import Encoder, Decoder                 # [PORT] was: networks.dreamer.vae
from .rnns import RSSMRepresentation, RSSMTransition  # [PORT] was: networks.dreamer.rnns
...
class DreamerModel(nn.Module):
    def __init__(self, config):
        ...
        # [PORT] upstream: only when ENV_TYPE == Env.STARCRAFT; always on here.
