"""Vendored from D:\\RL\\lzj\\MAPPO\\replay_buffer.py (mtime 2025-03-07). Algorithm logic intact."""
import numpy as np
import torch
...
class ReplayBuffer:
    ...
    def get_training_data(self):
        ...
        for key in self.buffer.keys():
            if key == 'a_n':
                batch[key] = torch.tensor(self.buffer[key], dtype=torch.long, device=self.device)
            else:
                batch[key] = torch.tensor(self.buffer[key], dtype=torch.float32, device=self.device)
        return batch
