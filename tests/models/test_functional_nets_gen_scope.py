...
LORA_R = 8
LORA = {"trans": 10816, "rew": 2689, "pred": 3463}            # = FILM + 256 * r
LORA_GROUPS = {"trans": [512, 10304], "rew": [512, 2177], "pred": [512, 2951]}
...
def _build_lora(net, lora_rank):
    if net == "trans":
        return FunctionalStateTransNet(LATENT, JOINT, HIDDEN, "lora_fc2", lora_rank)
    if net == "rew":
...
def test_lora_fc2_forward_shapes_no_nan():
    ...
    trans = _build_lora("trans", LORA_R)
    rew = _build_lora("rew", LORA_R)
    pred = _build_lora("pred", LORA_R)

    s_next = trans(state, action, torch.randn(B, LORA["trans"]))
...
def test_lora_fc2_zero_params_is_residual_identity():
    # Zero generated params -> Delta_W=0 (W2_eff=W2_base) AND head W=b=0 -> delta_s=0 ->
    # s_next=state. Confirms the residual path survives the shared-base + low-rank-delta split.
    B = 3
    trans = _build_lora("trans", LORA_R)
    state = torch.randn(B, LATENT)
    action = torch.zeros(B, JOINT)
    action[:, 0] = 1.0
    s_next = trans(state, action, torch.zeros(B, LORA["trans"]))
    assert torch.allclose(s_next, state, atol=1e-5)
