"""C7-EXT-SMOKE1 — MA-MuZero-GH Easy 20K env-step smoke gate (pkg-07 spec 06 §3.9).
...
§10.2 (MAPPO smoke). Gated by ``@pytest.mark.slow``.

Per spec 06 §3.9 if 20K proves insufficient on Easy in early
implementation, the budget may be raised to 40K with a synchronous spec
06 §9 + README C7-EXT-SMOKE1 footnote edit.
"""
...
@pytest.mark.slow
def test_ma_muzero_gh_easy_smoke_return_beats_random_at_20k():
    """C7-EXT-SMOKE1 MA-MuZero-GH smoke gate."""
